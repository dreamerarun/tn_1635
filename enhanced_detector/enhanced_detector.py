...
USE_HARDWARE = True  # Set to True when testing on real hardware
...
class EnhancedDetector(Node):
    ...
    def publish_stored_tfs(self):
        ...
        # Republish fertiliser
        if self.last_fertiliser_position is not None:
            x, y, z = self.last_fertiliser_position
            self.publish_tf(f"{TEAM_ID}_fertiliser_can", x, y, z)
    ...
    def detect_fertiliser_aruco(self, image_disp):
        """Detect fertiliser can using ArUco marker"""
        ...
        if ids is not None and len(ids) > 0:
            for i, marker_id in enumerate(ids.flatten()):
                ...
                self.fertiliser_position = base_pos

                # Check if this is a new detection
                if self.last_fertiliser_position is None:
                    self.get_logger().info(
                        f"✅ FERTILISER DETECTED: [{X:.3f}, {Y:.3f}, {Z:.3f}] (Marker ID: {marker_id})"
                    )

                self.last_fertiliser_position = (X, Y, Z)

                # Publish TF
                frame_id = f"{TEAM_ID}_fertiliser_can"
                self.publish_tf(frame_id, X, Y, Z)

                # Draw annotation
                cv2.circle(image_disp, (cx, cy), 10, (255, 0, 0), -1)
                cv2.putText(image_disp, f"Fertiliser (ID:{marker_id})", (cx - 60, cy - 20),
    ...
    def color_callback(self, msg):
        ...
        # Detect fertiliser can using ArUco
        image_disp = self.detect_fertiliser_aruco(image_disp)

        # Detect bad fruits
        image_disp = self.detect_bad_fruits(image_disp)

        # Status overlay
        status_text = f"Fruits: {len(self.detected_fruits)}/4 | Fertiliser: {'YES' if self.last_fertiliser_position else 'NO'}"
        ...
        if self.frame_count % 200 == 0:
            self.get_logger().info(
                ...
                f"Fertiliser: {'✓' if self.last_fertiliser_position else '✗'}"
            )
